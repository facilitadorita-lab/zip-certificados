...
async function abrirTicketSuporte(req, res, modulo) {
  try {
    ...
    if (!banco.ok) {
      console.error("Falha ao gravar ticket DLT:", banco.aviso || "erro desconhecido");
...
const downloadQueue = [];
const downloadFingerprints = new Map();
let downloadActiveCount = 0;

function atualizarPosicoesFilaDownloads() {
  downloadQueue.forEach((item, index) => {
    const job = downloadJobs.get(item.jobId);
    if (job) job.fila_posicao = index + 1;
  });
}

function criarChaveDownload({ ids, equipamentos, testeInicio, testeFim, usuario }) {
...
function limparDownloadsExpirados() {
  const agora = Date.now();
  for (const [chave, jobId] of downloadFingerprints) {
    ...
    if (!job || (expiraEm && expiraEm <= agora && !["pendente", "processando"].includes(job.status))) {
      downloadFingerprints.delete(chave);
    }
  }
  for (const [ticket, registro] of downloadTickets) {
    if (registro.expiraEm <= agora) downloadTickets.delete(ticket);
  }
}

function buscarDownloadDuplicado(chave) {
  limparDownloadsExpirados();
  const jobId = downloadFingerprints.get(chave);
...
function processarFilaDownloads() {
  while (downloadActiveCount < DOWNLOAD_MAX_CONCURRENT && downloadQueue.length) {
    const item = downloadQueue.shift();
    atualizarPosicoesFilaDownloads();
    const job = downloadJobs.get(item.jobId);
    if (!job) continue;
    downloadActiveCount++;
    job.fila_posicao = 0;
    ...
    processarDownloadMassa(item.jobId, item.registros)
      .catch(async (e) => {
        const atual = downloadJobs.get(item.jobId);
        if (!atual) return;
        atual.status = "erro";
        atual.erro = e.message;
        atual.atualizado_em = new Date().toISOString();
        await salvarDownloadJob(atual).catch(() => {});
        downloadFingerprints.delete(atual.chave_deduplicacao);
      })
      .finally(() => {
        downloadActiveCount--;
        atualizarPosicoesFilaDownloads();
        processarFilaDownloads();
      });
  }
}

function enfileirarDownload(jobId, registros) {
  const job = downloadJobs.get(jobId);
  if (!job) return;
  downloadQueue.push({ jobId, registros });
  atualizarPosicoesFilaDownloads();
  job.atualizado_em = new Date().toISOString();
  processarFilaDownloads();
}

const downloadCleanupTimer = setInterval(limparDownloadsExpirados, 10 * 60 * 1000);
...
async function listarHistoricoDownloadJobs(modulo = "DLT", limit = 50, solicitadoPor = null) {
  const persistidos = await listarDownloadJobsPersistidos(modulo, limit, solicitadoPor);
  const recentes = [...downloadJobs.values()].filter((job) => {
...
function montarFiltroBuscaCertificado(termos, normalizador, campoEquipamento) {
  ...
  for (const termo of termos) {
    ...
    // Bases antigas podem guardar o mesmo logger como "0163" ou "DLT-0163".
    const numero = String(termo || "").replace(/\D/g, "");
    if (numero) {
      const codigo = numero.padStart(4, "0");
      loggers.add(codigo);
      loggers.add(`DLT-${codigo}`);
    }
  }
...
function montarBaseLoggersDLT() {
  return Object.entries(MAPA_LOGGERS).map(([codigo, serie]) => ({
    modulo: "DLT",
    logger_codigo: normalizarDLT(codigo),
    logger: codigoCurtoLogger(codigo),
    serie_esperada: serie
  }));
}

async function buscarStatusAtualDLT(codigos = []) {
  const params = new URLSearchParams();
  params.set(
    "select",
  ...
  params.set("modulo", "eq.DLT");
  params.append("order", "data_movimentacao.desc");
  params.set("limit", "10000");
  if (codigos.length) {
    const variantes = new Set();
    for (const codigo of codigos) {
      const normalizado = normalizarDLT(codigo);
...
function combinarDisponibilidadeDLT(statusRegistros, opcoes = {}) {
  const statusPorLogger = new Map();
  for (const item of statusRegistros) {
    const chave = normalizarDLT(item.logger_codigo);
    if (chave && !statusPorLogger.has(chave)) statusPorLogger.set(chave, item);
  }

  const base = montarBaseLoggersDLT().map(item => {
  ...
  const lista = new Set((opcoes.lista || []).map(normalizarDLT).filter(Boolean));
...
function avaliarChecklistPreTesteDLT({ equipamento, disponibilidade, certificados, testeInicio, testeFim }) {
  const problemas = [];
  const avisos = [];
  ...
  if (!disponibilidade) {
    problemas.push("Logger nao cadastrado no mapa DLT.");
  ...
  if (bloqueados.length) {
    problemas.push(`${bloqueados.length} certificado(s) com reprovacao, erro ou divergencia.`);
  }
...
async function criarZipNoDisco(registros, zipPath, job) {
  ...
  const persistirProgresso = () => {
    ...
    persistencia = persistencia.then(() => salvarDownloadJob(job));
    return persistencia;
  };
...
app.get("/certificados", async (req, res) => {
  try {
    ...
    // `dlt` é a busca normal da tela e também aceita vários loggers, sem
    // mudar o modo da interface para "Buscar por lista".
    const listaEquipamentos = normalizarListaQuery(req.query.equipamentos || req.query.lista);
    const buscaDlt = normalizarListaQuery(req.query.dlt);
    ...
    const filtros = [];
    if (buscaDlt.length) {
      const filtroBusca = montarFiltroBuscaCertificado(buscaDlt, normalizarDLT, "dlt");
...
app.get("/loggers/disponibilidade", async (req, res) => {
  try {
    const lista = normalizarListaQuery(req.query.lista || req.query.equipamentos || req.query.dlt);
    const limit = limitarNumero(req.query.limit, 100, 1, 500);
    const offset = limitarNumero(req.query.offset, 0, 0, 100000);
    const status = await buscarStatusAtualDLT(lista);
    const combinado = combinarDisponibilidadeDLT(status.registros, {
...
app.post("/loggers/checklist-pre-teste", async (req, res) => {
  try {
    const equipamentos = Array.isArray(req.body?.equipamentos)
      ? req.body.equipamentos.map(String).filter(Boolean)
      : normalizarListaQuery(req.body?.equipamentos || req.body?.lista || req.body?.dlt);
    ...
    for (const certificado of certificados) {
      const chave = normalizarDLT(certificado.dlt);
      if (!chave) continue;
    ...
    const itens = unicos.map(equipamento => avaliarChecklistPreTesteDLT({
      equipamento,
...
app.post("/downloads/massa", async (req, res) => {
  try {
    ...
    const chaveDeduplicacao = criarChaveDownload({
      ids,
      equipamentos: listaEquipamentos,
      testeInicio,
      testeFim,
    ...
    const jobExistente = buscarDownloadDuplicado(chaveDeduplicacao);
    if (jobExistente) {
    ...
    if (downloadQueue.length >= DOWNLOAD_MAX_QUEUE) {
    ...
    const job = {
      ...
      fila_posicao: downloadQueue.length + 1,
      parametros: {
    ...
    downloadFingerprints.set(chaveDeduplicacao, jobId);
    await salvarDownloadJob(job);

    enfileirarDownload(jobId, Array.isArray(registros) ? registros : []);
...
app.get("/downloads/massa/historico", async (req, res) => {
  try {
    ...
    const solicitadoPor = role === "dev" || role === "administrador"
      ? null
      : req.auth?.user?.id;
    const jobs = await listarHistoricoDownloadJobs("DLT", limit, solicitadoPor);
    res.json({ jobs });
  } catch (e) {
    res.status(500).json({ erro: e.message });
  }
